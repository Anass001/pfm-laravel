        ...
        <div class="pics" style="display:flex;justify-content:center;width: 80vw;gap: 70px">
            <div class="left" style="width: 400px; margin-top: 70px">
                <form action="{{route('showResults')}}" method="POST" style="width: 230px">
                    @csrf
                    <input type="text" name="destination" class="form-control form-group"
                           placeholder="Type your destination" style="display:block">
                    <input type="date" name="check-in" class="form-control form-group" placeholder="Check in"
                           style="display:block">
                    <input type="date" name="check-out" class="form-control form-group" placeholder="Check out"
                           style="display:block">
                    <input type="number" name="Adults" class="form-control form-group" placeholder="Adults"
                           style="display:block">
                    <input type="number" name="Room" class="form-control form-group" placeholder="Rooms"
                           style="display:block">
            ...
            <div style="height: 100px">
                <div class="details" style="width:100%;height:70px">
                    ...
                    <div class="left" style="float:left">
                        ...
                        <h5>{{$property->city}}, {{$property->country}}</h5>
                    </div>
                ...
                <div class="right" style="display:flex;justify-content:center; gap: 10px">
                    <div class="right-pics" style="width: 300px">
                        @foreach ($property->images as $image)
                            <img style='width:150px; height:auto; margin-top: 10px; margin-bottom:10px'
                                 src='{{$image->image_url}}'
                                 alt="hotel photo">
                        @endforeach
                    </div>
                    <img src={{$property->images->first()->image_url}} alt="" style="width: auto ;height: 300px"/>
                </div>
            </div>
        </div>
        <div class="pics2" style="display:flex;justify-content:center;width: 80vw;gap: 70px; margin-top: 100px">
            <div class="left" style="width: 70%">
                <p style="word-wrap: break-word; margin-top: 50px">{{$property->description}}</p>
            </div>
                ...
                <div class="container justify-content-center mt-5 border-left border-right"
                    ...
                    @foreach ($property->reviews as $review)
                        <div class="d-flex justify-content-center py-2">
                            <div class="second py-2 px-2"><span class="text1">{{$review['description']}}</span>
                                <div class="d-flex justify-content-between py-1 pt-2">
                                    <div><img src="https://i.imgur.com/AgAC1Is.jpg" width="18"><span
                                            class="text2">@yield('logged_in')</span></div>
                                    <div><span class="text3">Upvote?</span><span class="thumbup"><i
                                                class="fa fa-thumbs-o-up"></i></span><span class="text4">3</span></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
